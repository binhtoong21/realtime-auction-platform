...
export function StatusBadge({ status, className = '' }) {
  ...
  // Maps backend status strings to human-readable labels
  const LABELS = {
    // Auction Statuses
    ACTIVE: 'Đang diễn ra',
    DRAFT: 'Bản nháp',
  ...
  // Maps backend status strings to CSS modifier classes matching our 6 semantic tokens
  const TOKEN_CLASSES = {
    // Auction Statuses
  ...
  const tokenClass = TOKEN_CLASSES[normalizedStatus] || 'status-badge--secondary';
